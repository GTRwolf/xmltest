...
*/
int add_XML(std::string path, std::string* addposion, int hierachy, std::string * attributes, int count_attributes,int flag)
{
    XMLDocument doc;
...
int main()
{
    ...
    median_blur->SetAttribute("width","10");
    ...
    //创建算法主节点
    XMLElement* ALgorithm1 = doc.NewElement("Algorithm1");
    doc.InsertEndChild(ALgorithm1);

    //创建算法分支点
    XMLElement* D11 = doc.NewElement("D11");
    ALgorithm1->InsertEndChild(D11);

    //创建滤波算法分支点
    XMLElement* D1_filter1 = doc.NewElement("D1_filter1");
    D11->InsertEndChild(D1_filter1);

    //创建中值滤波算法分支点
    XMLElement* median_blur1 = doc.NewElement("median_blur1");
    median_blur1->SetAttribute("width","10");
    median_blur1->SetAttribute("cut_boundary","3");
    D1_filter1->InsertEndChild(median_blur1);
    XMLElement* aaa = doc.NewElement("aaa");
    median_blur1->InsertEndChild(aaa);
    //const char* path_save = path.c_str();
    doc.SaveFile("path_save.xml",false);
    return 0;
}
